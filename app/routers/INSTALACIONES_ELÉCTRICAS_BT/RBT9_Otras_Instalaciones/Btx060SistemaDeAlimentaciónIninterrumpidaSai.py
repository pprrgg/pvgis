...
El presente informe tiene como objetivo proporcionar una guía técnica para la instalación de un Sistema de Alimentación Ininterrumpida (SAI). Se estimarán los valores necesarios basados en la documentación proporcionada y en las especificaciones técnicas estándar.

\section{Requisitos Previos}
Antes de proceder con la instalación, es necesario realizar un análisis de los requisitos técnicos y de seguridad:

\subsection{Evaluación de la Carga}
Determinar la carga total que el SAI deberá soportar. Esto incluye todos los dispositivos que se conectarán al SAI. Para ello, se debe calcular la potencia total en vatios (W) o voltamperios (VA) de los equipos.

\subsection{Espacio Físico}
Asegurar que el lugar de instalación cuente con el espacio suficiente para el SAI y sus componentes adicionales. El SAI debe colocarse en un área bien ventilada y libre de obstrucciones.

\subsection{Alimentación Eléctrica}
Verificar que la fuente de alimentación eléctrica cumpla con los requisitos del SAI. Esto incluye:
\begin{itemize}
    \item Voltaje de entrada: 220V AC (en la mayoría de los casos).
    \item Frecuencia: 50/60 Hz.
    \item Conexión a tierra: Es fundamental para garantizar la seguridad y el correcto funcionamiento del SAI.
\end{itemize}

\section{Instalación del SAI}
A continuación, se describen los pasos para la instalación del SAI:

\subsection{1. Montaje del Equipo}
\begin{itemize}
    \item Colocar el SAI en una superficie plana y estable.
    \item Asegurar que el equipo esté alejado de fuentes de calor o humedad.
    \item Dejar un espacio mínimo de 10 cm alrededor del SAI para una ventilación adecuada.
\end{itemize}

\subsection{2. Conexión Eléctrica}
\begin{itemize}
    \item Conectar el SAI a la toma de corriente principal.
    \item Verificar que la conexión a tierra esté correctamente instalada.
    \item Conectar los dispositivos críticos (servidores, routers, etc.) a las salidas del SAI.
\end{itemize}

\subsection{3. Configuración del Software (si aplica)}
Algunos SAI incluyen software de gestión para monitorear el estado de la batería y la carga. En caso de contar con esta funcionalidad:
\begin{itemize}
    \item Instalar el software en el equipo conectado al SAI.
    \item Configurar las alertas y opciones de apagado automático en caso de fallo de energía.
\end{itemize}

\section{ Valores}

\subsection{Capacidad del SAI}
\begin{itemize}
    \item Carga total estimada: 1500 VA.
    \item Tiempo de respaldo requerido: 30 minutos.
    \item Capacidad de la batería: 100 Ah (Amperios-hora).
\end{itemize}

\subsection{Requerimientos de Energía}
\begin{itemize}
    \item Voltaje de entrada: 220V AC.
    \item Frecuencia: 50 Hz.
    \item Consumo máximo: 1500 W.
\end{itemize}

\section{Conclusión}
La instalación de un SAI es un proceso crítico que requiere una planificación adecuada para garantizar su correcto funcionamiento. Este informe proporciona una guía básica para la instalación y estimación de valores basados en la documentación técnica proporcionada. Es recomendable seguir las especificaciones del fabricante y contar con personal calificado para la instalación.
...
La potencia del SAI será la suficiente para suministrar la potencia activa y reactiva de
las cargas a conectar, es decir, la potencia aparente necesaria (VA), aplicándoles (si
fuese necesario) un factor de simultaneidad. Esto es a lo que respecta a la potencia de
salida del SAI. Has de tener presente que los SAIs (al igual que cualquier receptor) tiene
un rendimiento (el cual dependerá de cada modelo). Pues bien, la potencia total del SAI,
también ha de considerar dicho rendimiento:
Potencia SAI = Potencia aparente cargas / rendimiento del SAI
Además, es aconsejable que el SAI no trabaje al 100% de forma permanente. Es
aconsejable sobredimensionarlo un 20%.
...
\begin{itemize}
    \item \href{https://www.idae.es/tecnologias/eficiencia-energetica/edificacion/sistema-de-alimentacion-ininterrumpida-sai}{IDAE Sistema de alimentación ininterrumpida (SAI)}
    \item \href{https://www.legrand.es/sites/g/files/ocwmcr651/files/2022-08/Guia-Tecnica-SAI-sistemas-de-alimentacion-ininterrumpida-Legrand.pdf}{LEGRAND SAI - GRUPOS ESTÁTICOS DE CONTINUIDAD}
...
@router.post("/Btx060")
def Btx060SistemaDeAlimentaciónIninterrumpidaSai(
    data: dict,
):
